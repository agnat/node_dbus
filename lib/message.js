var valid = require('./detail/valid')
  , sig_parser = require('./signature_parser')
  ;
...
var endianess = (/*littleEndian*/ true ? "I" : "B").charCodeAt(0); //  TODO

function padded(type_size, current_size) {
  return type_size + (current_size % type_size);
}

var header_signature = sig_parser.parse("yyyyuuai");

function wireSize(sig, args) {
  var size = 0;
  for (var i = 0; i < sig.length; ++i) {
    size += sig[i].paddedSize(size, args[i]);
  }
  return size;
}
Message.prototype._computeMessageSize = function() {
  var header_size = wireSize(header_signature,
      [endianess, this.type, this.flags, 1, 0, this.serial, this.headers]);
